...
def e2e_card($platform; $status):
    "<article class=\"e2e-card \(e2e_tone($status))\"><span class=\"e2e-platform\">\($platform)</span><strong>\(e2e_label($status))</strong><p>Safari → Signeur → Nostr relays → npub</p></article>";
...
| [
    ...
    "<meta name=\"description\" content=\"Signeur is a Nostr signer for iPhone and Mac. Explore the project and its live test coverage.\">",
    "<title>Signeur · Your keys. Your consent.</title>",
    "<link rel=\"icon\" type=\"image/png\" sizes=\"180x180\" href=\"signeur-icon.png\">",
    "<link rel=\"apple-touch-icon\" sizes=\"180x180\" href=\"signeur-icon.png\">",
    "<link rel=\"stylesheet\" href=\"coverage.css\">",
    "</head>",
    "<body>",
    "<div class=\"page-glow glow-one\"></div><div class=\"page-glow glow-two\"></div>",
    "<header class=\"site-header\">",
    "<a class=\"brand\" href=\"./\" aria-label=\"Signeur home\"><img src=\"signeur-icon.png\" alt=\"\"><span><strong>Signeur</strong><small>Nostr signer</small></span></a>",
    ("<nav><a href=\"#how-it-works\">How it works</a><a href=\"#nip46-test\">Test NIP-46</a><a href=\"#e2e-results\">E2E status</a><a href=\"#coverage\">Coverage</a><a class=\"github-icon-link\" href=\"https://github.com/guaka/signeur\" aria-label=\"GitHub repository\">" + github_mark + "</a></nav>"),
    "</header>",
    "<main>",
    "<section class=\"hero\">",
    "<h1>A Nostr signer <em>for iPhone and Mac.</em></h1>",
    "<p>Store Nostr keys on your device, pair clients over NIP-46, and grant each client only the permissions you want.</p>",
    "<div class=\"hero-actions\"><a class=\"primary-action\" href=\"#nip46-test\">Test NIP-46</a></div>",
    "</section>",
    "<section class=\"nip46-section section-block\" id=\"nip46-test\">",
    "<div class=\"section-heading nip46-heading\"><div><h2>See NIP-46 <em>in motion.</em></h2></div><p>Create a disposable test client, pair it with Signeur, and watch an encrypted public-key request make the round trip through Nostr relays.</p></div>",
    ...
    "<div class=\"flow-node\" data-flow-step=\"connected\"><span>3</span><strong>Signeur</strong><small>You review and approve</small></div><div class=\"flow-arrow return-arrow\" aria-hidden=\"true\">←</div>",
    ...
    "<div class=\"tester-intro\" id=\"nip46-intro\"><div><span class=\"tester-overline\">Safe to experiment</span><h3>Pair this page with Signeur.</h3><p>The page generates a disposable client key in your browser. It is kept only in memory and disappears when you reset or close the tab.</p></div><button class=\"primary-action tester-start\" id=\"nip46-start\" type=\"button\">Start NIP-46 test</button></div>",
    ...
    "<div class=\"pairing-card\"><div class=\"qr-frame\"><canvas id=\"nip46-qr\" width=\"272\" height=\"272\" aria-label=\"Nostr Connect QR code\"></canvas></div><div class=\"pairing-actions\"><a class=\"primary-action\" id=\"nip46-open-link\" href=\"#\">Open in Signeur</a><button class=\"secondary-action\" id=\"nip46-copy-link\" type=\"button\">Copy link</button></div><a class=\"raw-connect-link\" id=\"nip46-raw-link\" href=\"#\">Open raw nostrconnect:// link</a></div>",
    "<div class=\"pairing-guide\"><span class=\"tester-overline\">Pairing instructions</span><h3>Scan, approve, then approve once more.</h3><ol><li><span>1</span><div><strong>Open the connection</strong><p>Scan the QR in Signeur, or use the button on this device.</p></div></li><li><span>2</span><div><strong>Approve the app connection</strong><p>Check the client name and relays shown by Signeur.</p></div></li><li><span>3</span><div><strong>Approve “Read public key”</strong><p>The page asks for your public <code>npub</code>—never your private key.</p></div></li></ol><label for=\"nip46-link-value\">Nostr Connect link</label><textarea id=\"nip46-link-value\" rows=\"3\" readonly></textarea><dl class=\"connection-facts\"><div><dt>Temporary client</dt><dd id=\"nip46-client-key\">—</dd></div><div><dt>Relays</dt><dd id=\"nip46-relays\">—</dd></div></dl></div>",
    "</div>",
    ...
    "<div class=\"tester-error\" id=\"nip46-error\" role=\"alert\" hidden><div><strong id=\"nip46-error-title\">The test could not finish</strong><p id=\"nip46-error-text\"></p><code id=\"nip46-error-code\"></code></div><div class=\"tester-error-actions\"><button class=\"secondary-action\" id=\"nip46-copy-error\" type=\"button\">Copy error</button><button class=\"secondary-action\" id=\"nip46-retry\" type=\"button\">Try again</button></div></div>",
    "</div>",
    ...
    "<article class=\"product-card\"><span class=\"card-number\">01</span><span class=\"section-kicker\">Keys</span><h3>Add a key</h3><p>Generate a key or import an existing <code>nsec</code>. Signeur stores private keys in the system Keychain.</p></article>",
    ...
    "</section>",
    "<section class=\"security-callout\"><div><span class=\"section-kicker\">Key storage</span><h2>Keys stay in the device-only Keychain.</h2></div><p>Private keys are stored as biometric-protected Keychain items. Touch ID, Face ID, or the device passcode gates access before a secp256k1 operation runs in memory. Signeur does not currently keep Nostr keys or signing inside the Secure Enclave because Apple’s enclave does not support secp256k1. <a href=\"https://github.com/guaka/signeur/issues/18\">See the Secure Enclave investigation.</a></p></section>",
    ...
    "<div class=\"section-heading\"><p>SigneurCore test coverage from the latest successful main build.</p></div>",
    "</section>",
    ...
    "<div class=\"section-heading\"><div><h2>Coverage by area</h2></div><p>SigneurCore only. Tests, dependencies, compiler-generated functions, skipped regions, and explicitly documented non-testable code are excluded.</p></div>",
    "<div class=\"area-grid\">"
] + (
    $areas
    | map(
    ...
    "</section>",
    "</main>",
    "<footer><div class=\"footer-brand\"><img src=\"signeur-icon.png\" alt=\"\"><span><strong>Signeur</strong><small>Nostr signer for iPhone and Mac</small></span></div><p>Build \($generated) UTC · <a href=\"coverage.json\">Raw coverage data</a> · SwiftPM + llvm-cov · <a href=\"https://github.com/guaka/signeur/blob/main/LICENSE\">AGPL-3.0</a></p></footer>",
